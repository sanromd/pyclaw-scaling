...
def acoustics2D(finalt=1.0,use_petsc=True,outdir='./_output',solver_type='classic',disable_output=True,dm=[30,30],**kwargs):
    """
    ...
    t2 = MPI.Wtime()
    log = MPI.File.Open(MPI.COMM_WORLD, os.path.join(outdir,"results_"+str(size)+".log"), MPI.MODE_CREATE|MPI.MODE_WRONLY)
...
if __name__=="__main__":
    ...
    #out_folder = '/simdesk/clawpack/pyclaw/examples/acoustics_2d_variable/scaling_'+str(size)+'_'
    tb1call = MPI.Wtime()
    acoustics2D(finalt=(0.2/np.sqrt(size))/40,dm=[ncells,ncells],solver_type=solver,outdir=out_folder)
    PETSc.COMM_WORLD.barrier()

    tb2call = MPI.Wtime()    
    size = PETSc.Comm.getSize(PETSc.COMM_WORLD)
    rank = PETSc.Comm.getRank(PETSc.COMM_WORLD)
    acoustics2D(finalt=(0.2/np.sqrt(size))/40,dm=[ncells,ncells,ncells],outdir=out_folder)
    PETSc.COMM_WORLD.barrier()

    tb3call = MPI.Wtime()
    if rank in processList:
        funccall = "acoustics2D(finalt=(0.2/np.sqrt("+str(size)+"))/40,dm=["+str(ncells)+","+str(ncells)+"],solver_type="+solver+",outdir=out_folder)"
    ...
    if MPI.COMM_WORLD.rank==0:
        results=np.load(os.path.join(out_folder,'results_'+str(size)+'.npy'))
        ...
        np.save(os.path.join(out_folder,'results_'+str(size)),results)
